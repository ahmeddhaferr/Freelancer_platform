...
const Header = ({ isHomeSidebarOpen, setIsHomeSidebarOpen }) => {
  ...
  const showHomeSidebarButton =
    ["/", "/profile", "/dashboard", "/setting"].includes(location.pathname) &&
    isTabletOrMobile;
  ...
  return (
    <div className={styles.header}>
      ...
      {showHomeSidebarButton ? (
        <aside
          className={`${styles.mobileSidebar} ${isHomeSidebarOpen ? styles.open : ""}`}
        >
          ...
          <nav className={styles.mobileSidebarNav}>
            <Link to="/" onClick={() => setIsHomeSidebarOpen(false)}>
              Home
            </Link>
            <Link to="/dashboard" onClick={() => setIsHomeSidebarOpen(false)}>
              Dashboard
            </Link>
            <Link to="/setting" onClick={() => setIsHomeSidebarOpen(false)}>
              Settings
            </Link>
            <Link to="/profile" onClick={() => setIsHomeSidebarOpen(false)}>
              Edit Profile
            </Link>
          </nav>
          <div className={styles.mobileSidebarNotifications}>
            <h4>Notifications</h4>
            <p className={styles.mobileSidebarSubtitle}>
              You have updates from today and this week.
            </p>

            <p className={styles.mobileSidebarGroupTitle}>Today</p>
            {todayNotifications.map((today, index) => (
              <div
                key={`today-${today.name}-${index}`}
                className={styles.mobileNotificationItem}
              >
                <img src={today.img} alt="" />
                <p>
                  <span>{today.name}</span> {today.type}{" "}
                  <small>{today.time}</small>
                </p>
              </div>
            ))}
